...
def bit_shuffle(data_bytes, w, h, use_bytes=False):
    ...
    rest_tiles = np.pad(rest_tiles, ((0, 0), (0, easy_tiles.shape[-1] - rest_tiles.shape[-1])), mode='constant', constant_values=0)
...
unshuffle_fn = lambda data_bytes, w, h, dim, rest_size, rest_dim: bit_unshuffle(data_bytes, w, h, dim, rest_size, rest_dim, use_bytes=use_bytes)
...
if __name__ == "__main__":
    w = 1216
    h = 832
    data = bytearray(np.arange(0, w * h *3, dtype=np.uint8).tobytes())
    t = time.perf_counter()
    codes = fec_encode(data, w, h)
    e = time.perf_counter()
    print(f"Encoded in {e - t:.6f}s")
    t = time.perf_counter()
    dec, n_err = fec_decode(data, codes, w, h)
    e = time.perf_counter()
    print(f"Decoded in {e - t:.6f}s, n_err={n_err}")
    print(f"Equal: {dec == data}")

    orig_data = data.copy()
    for i in range(10):
        offset = np.random.randint(0, len(data) - 1)
        data[offset] = data[offset] ^ 0b11111111

    t = time.perf_counter()
    dec, n_err = fec_decode(data, codes, w, h)
    e = time.perf_counter()
    print(f"Decoded in {e - t:.6f}s, n_err={n_err}")
    print(f"Equal: {dec == orig_data}")